                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header border-bottom">
                                        <h4 class="card-title">Kegiatan <a href=" {{ route('backend-kegiatan.create') }} "
                                            ...
                                            <tbody>
                                                @foreach ($kegiatan as $key => $kegiatans)
                                                    <tr>
                                                        ...
                                                        <td> {{ $kegiatans->is_active == '0' ? 'Aktif' : 'Tidak Aktif' }}
                                                        </td>
                                                        <td>
                                                            <div class="d-flex">
                                                                <a  href=" {{ route('backend-kegiatan.edit', $kegiatans->id) }} "
                                                                    class="m-1 btn btn-success btn-sm">Edit</a>
                                                                <form method="POST"
                                                                    action="{{ route('backend-kegiatan.destroy', $kegiatans->id) }} ">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <button type="submit"
                                                                        class="m-1 btn btn-danger btn-sm">Hapus</button>
                                                            </div>
